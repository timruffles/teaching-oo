...
def get_province_name(provinces, code)
	if provinces.has_value?(code)
		return provinces.invert.fetch(code)
	else
		return "Invalid province code."
	end
end
puts "Input a province code:"
input = $stdin.gets.chomp
puts get_province_name(provinces, input)
...
def show_months(months, *month_numbers)
	month_numbers.each do |number|
		print months[number-1] + " "
	end
end
show_months(months, 1, 3, 12)